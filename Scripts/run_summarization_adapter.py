...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments,
         Seq2SeqTrainingArguments, MultiLingAdapterArguments)
    )
    ...
    # Calculate rouge score for every individual data(text)
    def calculate_individual_score(text, prediction, lable):
        index = []
        texts = []
        predicted_summary = []
        actual_summary = []
        ...
        for i in range(1, len(text)+1):
            ...
            txt = text[(i-1):(i)]
            actual = lable[(i-1):(i)]
            predicted = prediction[(i-1):(i)]

#             print("\n\n\################################ IN side for loop #########################\n\n")

#             print(f"{indx} \n{txt}\n {actual}\n {predicted} \n\n\n")

            result = metric.compute(predictions=predicted, references=actual)
            # print(result['rouge1'].mid.fmeasure)
            result1 = {key: value.mid.fmeasure *
...
#         print(f"{len(index)} \n{len(texts)}\n {len(predicted_summary)}\n {len(actual_summary)}\n \n\n\n")
        return index, texts, predicted_summary, actual_summary, r1, r2, rL, rLSum
...
##############################......Save Results for each of the sentence!!!!!!!.......###################################

                # Change text and lable according to dataset!!!
                index, text, predicted_summary, actual_summary, r1, r2, rL, rLSum = calculate_individual_score(
                    raw_datasets['test'][text_column], predictions, raw_datasets['test'][summary_column])

#                 print("\n\n\################################ After calling Function #########################\n\n")
#                 print(f"{len(index)} \n{len(text)}\n {len(predicted_summary)}\n {len(actual_summary)}\n \n\n\n")

                headline = ('Index', 'Text', 'Predicted HL', 'Actual HL',
                            'Rouge1', 'Rouge2', 'RougeL', 'RougeLsum')
                data = [index, text, predicted_summary,
                        actual_summary, r1, r2, rL, rLSum]
                export_data = zip_longest(*data, fillvalue='')
                output_prediction_file = os.path.join(
                    training_args.output_dir, "Individual_Score.csv")

                with open(output_prediction_file, 'w', newline='') as file:
